...
const PracticePlaceList = () => {
    ...
    const [deleteDialog, setDeleteDialog] = useState({ isOpen: false, practiceId: null, practiceName: '' });
    ...
    const fetchPracticePlaces = async () => {
        try {
            ...
            setPracticePlaces(response.data || []);
        } catch (err) {
        ...
        <div className="dashboard">
            <div className="dashboard-header">
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    ...
                    <RoleGuard allowedRoles={['ADMIN']}>
                        <button onClick={() => navigate('/practice-places/add')} className="btn-primary">
            ...
            {loading ? (
                ...
                <div className="auth-card" style={{ textAlign: 'center', padding: '3rem' }}>
                    <p style={{ color: 'var(--text-muted)' }}>Belum ada data tempat praktik</p>
                    <RoleGuard allowedRoles={['ADMIN']}>
                        <button onClick={() => navigate('/practice-places/add')} className="btn-primary" style={{ marginTop: '1rem' }}>
                    ...
                    {practicePlaces.map((place) => (
                        <div key={place.practice_id} className="auth-card">
                            <div style={{ marginBottom: '1rem' }}>
                                <h3 style={{ fontSize: '1.25rem', marginBottom: '0.5rem' }}>{place.nama_praktik}</h3>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                                ...
                                <RoleGuard allowedRoles={['ADMIN']}>
                                    <button
                                        onClick={() => navigate(`/practice-places/${place.practice_id}/edit`)}
                                        className="btn-primary"
                                        style={{
                                            flex: 1,
                                            padding: '0.5rem',
                                    ...
                                    <button
                                        onClick={() => setDeleteDialog({ isOpen: true, practiceId: place.practice_id, practiceName: place.nama_praktik })}
                                        className="btn-primary"
                                        style={{
                                            flex: 1,
                                            padding: '0.5rem',
            ...
            <ConfirmDialog
                ...
                onClose={() => setDeleteDialog({ isOpen: false, practiceId: null, practiceName: '' })}

import { beforeEach, describe, expect, it, vi } from 'vitest';
import { render, screen } from '@testing-library/react';
import ImunisasiForm from './ImunisasiForm';

const mockNavigate = vi.fn();

vi.mock('react-router-dom', async () => {
  const actual = await vi.importActual('react-router-dom');
  return {
    ...actual,
    useNavigate: () => mockNavigate,
    useParams: () => ({}),
  };
});

vi.mock('../../context/AuthContext', () => ({
  useAuth: () => ({
    user: {
      role: 'USER',
      position_user: 'bidan_praktik',
    },
  }),
}));

vi.mock('../../services/api', () => ({
  createImunisasi: vi.fn(),
  getImunisasiDetail: vi.fn(),
  getPasienList: vi.fn(),
  getPracticePlaces: vi.fn(),
  updateImunisasi: vi.fn(),
}));

import { getPasienList, getPracticePlaces } from '../../services/api';

describe('ImunisasiForm', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getPasienList.mockResolvedValue({ data: [] });
    getPracticePlaces.mockResolvedValue({ data: [] });
  });

  it('renders refreshed light section cards', async () => {
    const { container } = render(<ImunisasiForm />);

    expect(screen.getByText('Input Data Imunisasi')).toBeInTheDocument();
    expect(await screen.findByText('Informasi Pasien')).toBeInTheDocument();
    expect(container.querySelector('.imunisasi-form__section-card')).toBeInTheDocument();
  });
});
